...
provide 和 inject 绑定并不是可响应的 使用2.6最新API Vue.observable 优化响应式 provide(推荐)
...
```
## 五、eventBus $emit/$on
通过一个空的Vue实例作为中央事件总线（事件中心），用它来触发事件和监听事
...
+ `$attrs` 接收除了props声明外的所有绑定属性（class、style除外） 将事件监听器继续向下传递
+ `$listeners` 接收除了带有.native事件修饰符的所有事件监听器 将事件监听器继续向下传递
