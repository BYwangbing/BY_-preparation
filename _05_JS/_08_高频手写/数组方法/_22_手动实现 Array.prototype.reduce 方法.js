...
* 归并 与之前的迭代不同，归并不是对每一项都执行目标函数，而是可以概括为如下两步：
    ...
    返回最终结果
* */
function _reduce(array, reduceCallback, initialValue) {
    if (!Array.isArray(array) || typeof reduceCallback !== 'function' || !array.length) {
        return [];
    }
    ...
    for (let i = hasInitialValue ? 1 : 0; i < array.length; i++) {
        value = reduceCallback(value, array[i], i, array);l
    }
    return value;
}
